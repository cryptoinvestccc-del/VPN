...
export function LineChart({ points, unit, title, height = 190 }: Props) {
  ...
  const shown = useRef<{ min: number; span: number } | null>(null)
  ...
  // Slide the new line in from where the old one stood, and when the
  // scale changes, ease the curve to it instead of snapping. The frames
  // touch the DOM directly: re-rendering the whole chart 60 times a
  // second would cost far more than moving one group and one dot. The
  // motion is a few pixels a second, so it stays on for reduced motion.
  useLayoutEffect(() => {
    const mover = moverRef.current
    const dot = dotRef.current
    if (!geom || !mover) return
    const start = geom.shift * geom.stepPx + leftover.current
    const from = shown.current ?? { min: geom.min, span: geom.span }
    const right = PAD.left + geom.plotW
    const floor = PAD.top + geom.plotH
    const place = (offset: number, eased: number) => {
      const min = from.min + (geom.min - from.min) * eased
      const span = from.span + (geom.span - from.span) * eased
      leftover.current = offset
      shown.current = { min, span }
      // Maps the target scale onto the one on screen right now.
      const a = geom.span / span
      const b = floor - a * floor - ((geom.min - min) * geom.plotH) / span
      mover.setAttribute(
        'transform',
        `matrix(1 0 0 ${a.toFixed(4)} ${offset.toFixed(2)} ${b.toFixed(2)})`,
      )
      dot?.setAttribute('cy', (a * curveY(geom.segs, right - offset) + b).toFixed(2))
    }
    place(start, 0)
    const rescale = from.min !== geom.min || from.span !== geom.span
    if (start <= 0 && !rescale) return
    const duration = Math.max(STEP_MS, (start / geom.stepPx) * STEP_MS)
    const t0 = performance.now()
    let frame = requestAnimationFrame(function tick(now) {
      const done = Math.min(1, (now - t0) / duration)
      place(start * (1 - done), 1 - (1 - done) ** 3)
...
function curveY(segs: Seg[], x: number): number {
  ...
  const u = 1 - t
  return u * u * u * s.y0 + 3 * u * u * t * s.c1 + 3 * u * t * t * s.c2 + t * t * t * s.y1
}
