...
пакетов вплоть до jumbo, молчание порта в ответ на пробинг, реальное
TLS-рукопожатие, переподключение после перезапуска сервера, отказ при
несовпадении pin и чистое завершение по SIGTERM.
...
шаблоны — впиши туда IP сервера и этот pin. Сертификат самоподписанный:
доверие устанавливается не через публичный CA, а через pinning отпечатка
(как в Trojan/ShadowTLS) — клиент откажется соединяться, если отпечаток
не совпадёт (защита от MITM).

**PSK в TLS-режиме не нужен.** Ключ обфускации выводится автоматически
из самого TLS-соединения (RFC 5705 keying material export — тот же
механизм, которым TLS 1.3 экспортирует производные ключи для внешних
протоколов). Единственный секрет, который нужно перенести с сервера на
клиента вручную — это `pinned_cert_sha256`. Хочешь отдельный статический
ключ поверх этого — можно задать `psk`/`psk_previous`, как в UDP-режиме;
это не обязательно.

## Ротация PSK без даунтайма (UDP-режим, или TLS со статическим psk)
